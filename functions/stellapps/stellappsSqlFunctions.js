...
// function insertStellappsDataCheckDuplicate(FormId, fields, contact_number, airtableBaseNo) {
const insertStellappsDataCheckDuplicate = (FormId, fields, contact_number, airtableBaseNo) => {
    return new Promise((resp, rej) => {
        try {
            cloudsql.query(
                "INSERT INTO stellappsData (FormId, data, contact_number,airtableBaseNo) VALUES (?, ? ,?, ?);",
                [FormId, JSON.stringify(fields), contact_number, airtableBaseNo],
                (err, result) => {
                    if (err) {
                        if (err && err.code === "ER_DUP_ENTRY") {
                            console.log("Duplicate data", contact_number);
                            cloudsql.query("INSERT INTO stellappsData_duplicate (FormId,data,contact_number,airtableBaseNo) VALUES (?, ?, ?, ?);",
...
const updateDataIntoDb = (FormId, fields, mobile_number, recId) => {
    return new Promise((resp, rej) => {
        cloudsql.query(
            `UPDATE stellappsData SET data=?, contact_number=?,  airtableId=? WHERE FormId=${FormId};`,
...
const updateStellappsDataSql = (fields) => {
    return new Promise((resp, rej) => {
        cloudsql.query(
            `UPDATE stellappsData SET data=? WHERE FormId=${fields.FormId};`,
...
const getStellappsDataSql = (formId) => {
    return new Promise((resp, rej) => {
        cloudsql.query(
            `SELECT data, airtableId FROM stellappsData WHERE FormId =${formId}`,
            (err, result) => {
                if (err) {
                    console.log("FAILED_UPDATE", JSON.stringify(err));
                    rej(err);
...
const updateStellappsAirtableIdSql = (formId, airtableID) => {
    return new Promise((resp, rej) => {
        cloudsql.query(
            `UPDATE stellappsData SET airtableId=? WHERE FormId=${formId};`,
            [airtableID],
            (err, result) => {
                if (err) {
                    console.log("FAILED_UPDATE", JSON.stringify(err));
...
//SELECT data FROM stellappsData WHERE data_cleaned IS NULL ORDER BY Id DESC LIMIT 250
function getRow() {
    return new Promise((resp, rej) => {
        try {
            cloudsql.query(
                `SELECT data FROM stellappsData WHERE data_cleaned IS NULL ORDER BY FormId DESC LIMIT 250;`,
                (err, result) => {
                    if (err) {
                        console.log(err);
                        rej(err);
...
function changeCleanedStatus(id) {
    ...
    return new Promise((resp, rej) => {
        try {
            cloudsql.query(
                `UPDATE stellappsData SET data_cleaned=? WHERE Id=${id};`,
                [2],
                (err, result) => {
                    if (err) {
                        console.log(err);
