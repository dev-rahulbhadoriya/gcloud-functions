...
const { getAttendanceDataSql } = require("./attendanceSqlFunctions");

function insertToAttendanceDataAirtable(formId, status) {
  return new Promise((resp, rej) => {
    getAttendanceDataSql(formId)
      .then((result) => {
        console.log(
          "INSERT",
        ...
        // sleep(1000);
        let base = attendanceDataInternalBases[status];
        let table = base("Attendance From Spoors");
        table.create(
...
function updateAttendanceDataAirtable(formId, recordId, status) {
  return new Promise((resp, rej) => {
    getAttendanceDataSql(formId)
      .then((result) => {
        console.log(
...
function checkAttendanceDataPreviousData(formId, status) {
...
function checkAttendanceDataQaStatus(recId, status) {
